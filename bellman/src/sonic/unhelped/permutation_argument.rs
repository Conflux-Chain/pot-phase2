...
impl<E: Engine> PermutationArgument<E> {
    ...
    pub fn make_argument(
            ...
            .zip(self.permutations.into_iter())
        {
            ...
            assert_eq!(
                s_product, s_prime_product,
                "product of coefficients must be the same"
            );
        ...
        for (a, b) in grand_products.iter() {
            let (c_a, c_b) =
                GrandProductArgument::commit_for_individual_products(&a[..], &b[..], &srs);
...

        {
            ...
            let valid = WellformednessArgument::verify(
                n,
    ...
    pub fn verify_s_prime_commitment(
        ...
        let h_term = multiexp(
            Some(s_r).iter().chain_ext(Some(p2_r).iter()),
            randomness.iter(),
        )
        .into_affine();
    ...
    pub fn make_signature(
        ...
        let s_prime_commitments_opening =
            argument.open_commitments_to_s_prime(&challenges, y, z_prime, &srs);

        let (proof, grand_product_signature) = {
            let (proof, grand_product_signature) =
                argument.make_argument_with_transcript(&mut transcript, y, z, &srs);
    ...
    // Argument a permutation argument. Current implementation consumes, cause extra arguments are required
    pub fn make_argument_with_transcript(
        self,
        ...
        let grand_product_signature =
            GrandProductArgument::create_signature(transcript, grand_products, y, z, &srs);
...
fn test_permutation_argument() {
    ...
    let valid = PermutationArgument::verify_s_prime_commitment(
        n,
        &randomness,
        &challenges,
        &s_prime_commitments,
