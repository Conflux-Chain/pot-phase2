...
pub fn create_aggregate_on_srs<E: Engine, C: Circuit<E>, S: SynthesisDriver>(
    ...
    create_aggregate_on_srs_using_information::<E, C, S>(
        circuit,
        inputs,
        srs,
        specialized_srs,
...
pub fn create_aggregate_on_srs_using_information<E: Engine, C: Circuit<E>, S: SynthesisDriver>(
    ...
    println!(
        "Commit and opening of for s(z, w) taken {:?}",
        start.elapsed()
    );
    ...
    let signature =
        PermutationArgument::make_signature(non_permuted_coeffs, permutations, w, z, &srs);
    ...
    println!(
        "Re-Evaluation and re-opening of s(z, Y) taken {:?}",
        start.elapsed()
    );
