...
pub fn create_permutation_structure<E: Engine, C: Circuit<E>>(
    ...
    let (a, b, c) = {
        let mut cs: PermutationSynthesizer<E, &'_ mut Preprocess<E>> =
            PermutationSynthesizer::new(&mut backend);
        ...
        match (
            one,
            <PermutationSynthesizer<E, &'_ mut Preprocess<E>> as ConstraintSystem<E>>::ONE,
        ) {
...
impl<E: Engine> PermutationStructure<E> {
    ...
    pub fn create_inverse_permutation_vectors(&self) -> (Vec<Vec<E::Fr>>, Vec<Vec<usize>>) {
        ...
        for i in 0..m {
            ...
            for (p, c) in permutations[i]
                .iter_mut()
                .zip(non_permuted_coeffs[i].iter())
            {
                if *p == 0 {
                    assert!(c.is_zero());
                } else {
    ...
    pub fn print_constraints(
        ...
        for constraint_idx in 1..=q {
            println!(
                "Constraint {} (term for Y^{})",
                constraint_idx, constraint_idx
            );
            let mut terms = vec![];
            for p_idx in 0..m {
                if let Some(variable_idx) = permutations[p_idx]
                    .iter()
                    .position(|&s| s == constraint_idx)
                {
    ...
    pub fn create_permutation_special_reference(&self, srs: &SRS<E>) -> SpecializedSRS<E> {
        ...
        let specialized_srs =
            PermutationArgument::make_specialized_srs(&non_permuted_coeffs, &permutations, &srs);
    ...
    pub fn make_signature(
        ...
        let signature =
            PermutationArgument::make_signature(non_permuted_coeffs, permutations, y, z, &srs);
...
fn test_simple_succinct_sonic() {
    ...
    impl<E: Engine> Circuit<E> for MyCircuit {
        fn synthesize<CS: ConstraintSystem<E>>(&self, cs: &mut CS) -> Result<(), SynthesisError> {
            ...
            cs.enforce_zero(
                LinearCombination::zero() + (Coeff::Full(E::Fr::from_str("2").unwrap()), a) - b,
            );
            cs.enforce_zero(
                LinearCombination::zero() + (Coeff::Full(E::Fr::from_str("20").unwrap()), a) - c,
            );
            cs.enforce_zero(
                LinearCombination::zero() + (Coeff::Full(E::Fr::from_str("10").unwrap()), b) - c,
            );
