...
use super::permutation_argument::{
    PermutationArgument, PermutationArgumentProof, PermutationProof, SpecializedSRS,
};
    ...
    pub fn add_aggregate(
        ...
        let szw = {
...

            {
                ...
                self.batch
                    .add_opening(s2_proof.d_opening, random, y_inv_by_x);
...

            {
                ...
                // using batching with random challenges and extra randomness for pairing equation
                {
                    ...
                    let p2_r = multiexp(self.s1_special_reference.p_2.iter(), challenges.iter())
                        .into_affine();
                    ...
                    self.batch
                        .add_opening(perm_proof.e_opening, random, z_prime);
                    ...
                    self.batch
                        .add_opening(perm_proof.f_opening, random, y_z_prime);
                    self.batch.add_opening_value(random, value);
...

                {
                    ...
                    self.batch.add_opening(
                        aggregate.signature.perm_argument_proof.s_opening,
                        random,
                        z,
                    );
                    self.batch.add_opening_value(random, value);
                    self.batch
                        .add_commitment(s_commitment.into_affine(), random);
                }
                ...
                {
                    let randomness: Vec<E::Fr> = (0..aggregate.signature.s_commitments.len())
                        .map(|_| self.randomness_source.gen())
                        .collect();
                    ...
                    assert!(
                        valid,
                        "grand product arguments must be valid for individual commitments"
                    );
                }
...

                {
...

                    {
                        ...
                        self.batch.add_opening(
                            aggregate.signature.grand_product_signature.proof.e_opening,
                            random,
                            z_inv,
                        );
                        self.batch.add_opening_value(
                            random,
                            aggregate.signature.grand_product_signature.proof.e_zinv,
                        );
...

                    {
                        ...
                        self.batch.add_opening(
                            aggregate.signature.grand_product_signature.proof.f_opening,
                            random,
                            w,
                        );
                        self.batch.add_opening_value(
                            random,
                            aggregate.signature.grand_product_signature.proof.f_y,
                        );
...

                    {
                        ...
                        self.batch.add_opening(
                            aggregate.signature.grand_product_signature.proof.t_opening,
                            random,
                            z,
                        );
                        self.batch.add_opening_value(random, t_zy);
                        self.batch.add_commitment(
                            aggregate.signature.grand_product_signature.t_commitment,
                            random,
                        );
                    }
                }
