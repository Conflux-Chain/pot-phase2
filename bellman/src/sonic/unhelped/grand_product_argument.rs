...
impl<E: Engine> GrandProductArgument<E> {
    pub fn create_signature(
        ...
        let wellformedness_signature =
            WellformednessArgument::create_signature(all_polys, wellformed_challenges, &srs);
        ...
        let t_commitment =
            grand_product_argument.commit_to_t_polynomial(&grand_product_challenges, y, &srs);
        let grand_product_openings =
            grand_product_argument.open_commitments_for_grand_product(y, z, &srs);
        let a_zy: Vec<E::Fr> = grand_product_openings
            .iter()
            .map(|el| el.0.clone())
            .collect();
        let proof =
            grand_product_argument.make_argument(&a_zy, &grand_product_challenges, y, z, &srs);
    ...
    pub fn commit_for_individual_products(
        a: &[E::Fr],
        b: &[E::Fr],
        srs: &SRS<E>,
    ...
    pub fn open_commitments_for_grand_product(
        &self,
        y: E::Fr,
        z: E::Fr,
        srs: &SRS<E>,
        ...
        for a_poly in self.a_polynomials.iter() {
            ...
            let opening = polynomial_commitment_opening(
                0,
                2 * n + 1,
                Some(constant_term).iter().chain_ext(a_poly.iter()),
                yz,
                &srs,
            );
    ...
    pub fn commit_to_individual_c_polynomials(&self, srs: &SRS<E>) -> Vec<(E::G1Affine, E::Fr)> {
        ...
        for (p, v) in self.c_polynomials.iter().zip(self.v_elements.iter()) {
            ...
            let c =
                multiexp(srs.g_positive_x_alpha[0..two_n_plus_1].iter(), p.iter()).into_affine();
    ...
    pub fn commit_to_t_polynomial(
        ...
        let c = multiexp(
            srs.g_negative_x_alpha[0..(2 * n + 1)]
                .iter()
                .rev()
                .chain_ext(srs.g_positive_x_alpha[0..(2 * n + 1)].iter()),
            t_polynomial[0..(2 * n + 1)]
                .iter()
                .chain_ext(t_polynomial[(2 * n + 2)..].iter()),
        )
        .into_affine();
...
fn test_grand_product_argument() {
    ...
    let b_commitment =
        multiexp(srs.g_positive_x_alpha[0..n].iter(), permutation.iter()).into_affine();

    let (a, b) =
        GrandProductArgument::commit_for_individual_products(&coeffs[..], &permutation[..], &srs);
    ...
    let a_zy: Vec<Fr> = grand_product_openings
        .iter()
        .map(|el| el.0.clone())
        .collect();
