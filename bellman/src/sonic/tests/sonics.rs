...
fn test_sonic_mimc() {
...

    {
        ...
        let start = Instant::now();
        let proof =
            create_proof_on_srs::<Bls12, _, Basic>(&AdaptorCircuit(circuit.clone()), &srs).unwrap();
        println!("done in {:?}", start.elapsed());

        println!("creating advice");
        let start = Instant::now();
        let advice =
            create_advice_on_srs::<Bls12, _, Basic>(&AdaptorCircuit(circuit.clone()), &proof, &srs)
                .unwrap();
        println!("done in {:?}", start.elapsed());
        ...
        let proofs: Vec<_> = (0..samples)
            ...
            .collect();
        let aggregate = create_aggregate_on_srs::<Bls12, _, Basic>(
        ...
        {
            let rng = thread_rng();
            let mut verifier = MultiVerifier::<Bls12, _, Basic, _>::new(
                AdaptorCircuit(circuit.clone()),
                &srs,
                rng,
            )
...
fn test_sonic_mimc_in_permutation_driver() {
...

    {
        ...
        let start = Instant::now();
        let proof =
            create_proof_on_srs::<Bls12, _, Permutation3>(&AdaptorCircuit(circuit.clone()), &srs)
                .unwrap();
        println!("done in {:?}", start.elapsed());

        println!("creating advice");
        let start = Instant::now();
        let advice = create_advice_on_srs::<Bls12, _, Permutation3>(
            &AdaptorCircuit(circuit.clone()),
            &proof,
            &srs,
        )
        ...
        let proofs: Vec<_> = (0..samples)
            .map(|_| (proof.clone(), advice.clone()))
            .collect();
        let aggregate = create_aggregate_on_srs::<Bls12, _, Permutation3>(
            &AdaptorCircuit(circuit.clone()),
            &proofs,
            &srs,
        );
        ...
        {
            let rng = thread_rng();
            let mut verifier = MultiVerifier::<Bls12, _, Permutation3, _>::new(
                AdaptorCircuit(circuit.clone()),
                &srs,
                rng,
            )
...
fn test_succinct_sonic_mimc() {
...

    {
        ...
        let perm_structure =
            create_permutation_structure::<Bls12, _>(&AdaptorCircuit(circuit.clone()));
        ...
        let info = get_circuit_parameters_for_succinct_sonic::<Bls12, _>(circuit.clone())
            .expect("Must get circuit info");
        println!("{:?}", info);
...

        {
            ...
            let mut verifier = SuccinctMultiVerifier::<Bls12, _, Permutation3, _>::new(
                AdaptorCircuit(circuit.clone()),
                &srs,
                rng,
            )
...
fn test_inputs_into_sonic_mimc() {
...

    {
        ...
        let info =
            get_circuit_parameters::<Bn256, _>(circuit.clone()).expect("Must get circuit info");
        println!("{:?}", info);

        println!("creating proof");
        let start = Instant::now();
        let proof =
            create_proof_on_srs::<Bn256, _, Basic>(&AdaptorCircuit(circuit.clone()), &srs).unwrap();
        println!("done in {:?}", start.elapsed());

        println!("creating advice");
        let start = Instant::now();
        let advice =
            create_advice_on_srs::<Bn256, _, Basic>(&AdaptorCircuit(circuit.clone()), &proof, &srs)
                .unwrap();
        println!("done in {:?}", start.elapsed());
        ...
        let proofs: Vec<_> = (0..samples)
            .map(|_| (proof.clone(), advice.clone()))
            .collect();
        let aggregate = create_aggregate_on_srs::<Bn256, _, Basic>(
            &AdaptorCircuit(circuit.clone()),
            &proofs,
            &srs,
        );
        ...
        {
            let rng = thread_rng();
            let mut verifier = MultiVerifier::<Bn256, _, Basic, _>::new(
                AdaptorCircuit(circuit.clone()),
                &srs,
                rng,
            )
...
fn test_high_level_sonic_api() {
...

    {
...

        {
            ...
            assert_eq!(
                verify_aggregate(
                    &vec![(proof.clone(), advice.clone()); 100],
                    &aggregate,
                    &vec![vec![image.clone()]; 100],
                    circuit.clone(),
                    rng,
