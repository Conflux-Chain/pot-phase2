    ...
    C: Circuit<E>,
{
    ...
    let (num_inputs, num_aux, num_constraints) = {
        let mut cs: NonassigningSynthesizer<E, &'_ mut Preprocess<E>> =
            NonassigningSynthesizer::new(&mut preprocess);
        ...
        match (
            one,
            <NonassigningSynthesizer<E, &'_ mut Preprocess<E>> as SonicConstraintSystem<E>>::ONE,
        ) {
    ...
    C: Circuit<E>,
{
    ...
    let (num_inputs, num_aux, num_constraints) = {
        let mut cs: PermutationSynthesizer<E, &'_ mut Preprocess<E>> =
            PermutationSynthesizer::new(&mut preprocess);
        ...
        match (
            one,
            <PermutationSynthesizer<E, &'_ mut Preprocess<E>> as SonicConstraintSystem<E>>::ONE,
        ) {
            (SonicVariable::A(1), SonicVariable::A(1)) => {}
            _ => return Err(SynthesisError::UnconstrainedVariable),
        }
...
pub fn generate_srs<E: Engine>(alpha: E::Fr, x: E::Fr, d: usize) -> Result<SRS<E>, SynthesisError> {
    ...
    fn eval<E: Engine>(
        ...
        worker.scope(g_x.len(), |scope, chunk| {
                ...
                .zip(h_x_alpha.chunks_mut(chunk))
            {
                ...
                scope.spawn(move |_| {
                    for ((((x, g_x), g_x_alpha), h_x), h_x_alpha) in x
                        .iter()
                        .zip(g_x.iter_mut())
    ...
    let g_negative_x_alpha = g_negative_x_alpha
        .into_iter()
        .map(|e| e.into_affine())
        .collect();
    let g_positive_x_alpha = g_positive_x_alpha
        .into_iter()
        .map(|e| e.into_affine())
        .collect();
