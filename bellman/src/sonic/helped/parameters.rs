...
impl<E: Engine> VerifyingKey<E> {
    pub fn new<C: SonicCircuit<E>, S: SynthesisDriver>(
        circuit: C,
        srs: &SRS<E>,
...
fn parameters_generation() {
    ...
    use super::{
        generate_parameters, generate_parameters_on_srs_and_information, generate_srs,
        get_circuit_parameters,
    };
    use rand::{thread_rng, Rand, Rng};

    let info = get_circuit_parameters::<Bls12, _>(MySillyCircuit { a: None, b: None })
    ...
    let params =
        generate_parameters::<Bls12, _>(MySillyCircuit { a: None, b: None }, alpha, x).unwrap();
    ...
    let params_on_srs =
        generate_parameters_on_srs_and_information::<Bls12>(&srs, info.clone()).unwrap();
